...
func _ready():
	connect('reflect', self, 'reflect_detection')
...
func reflect_detection(starting_point, cast_to_vector):
	...
	print(new_ray.get_collider())
	
	var t = laser_trail.instance()
	var d = starting_point - new_ray.get_collision_point()
	print(d.length())
	t.position = starting_point
	t.region_rect = Rect2(0,0,d.length(),2)
	var normal_vect = Vector2(1,0)
	var angle_in_rads = normal_vect.angle_to(new_ray.cast_to)
